...
if __name__ == "__main__":
  ...
  function scrapeTestCases() {
    ...
    preElements.forEach(pre => {
      ...
      const parentText = pre.parentNode ? pre.parentNode.innerText || '' : '';
      const parentId = pre.parentNode?.id || '';
      const preId = pre.id || '';
      const preClass = pre.className || '';
      
      const isInput = (
        parentText.toLowerCase().includes('sample input') ||
        parentText.toLowerCase().includes('example input') ||
        preId.toLowerCase().includes('input') ||
        parentId.toLowerCase().includes('input') ||
        preClass.toLowerCase().includes('input')
      );
      
      const isOutput = (
        parentText.toLowerCase().includes('sample output') ||
        parentText.toLowerCase().includes('example output') ||
        preId.toLowerCase().includes('output') ||
        parentId.toLowerCase().includes('output') ||
        preClass.toLowerCase().includes('output')
      );
    ...
    if (inputs.length === 0 && preElements.length > 0) {
      preElements.forEach(pre => {
        const text = pre.innerText || pre.textContent;
        if (text && /^d+/.test(text.trim()) && text.trim().split('n').length > 1) {
          inputs.push(text.trim());
        }
      });
    }
    ...
    for (let i = 0; i < Math.max(inputs.length, outputs.length); i++) {
      scraped.push({ 
        input: inputs[i] || '', 
        expected: outputs[i] || '', 
        isCustom: false, status: 'pending', actual: null, stderr: null, timeMs: null 
      });
    }
    return scraped;
  }
        ...
        chrome.storage.local.get([urlKey], (res) => {
          ...
          if (res[urlKey] && Array.isArray(res[urlKey])) {
            res[urlKey].forEach(tc => {
              localTestCases.push({ ...tc, isCustom: true, status: 'pending', actual: null, stderr: null, timeMs: null });
            });
          }
          renderTestCases();
        });
  ...
  function renderTestCases() {
    ...
    document.getElementById('behelith-save-custom-btn').onclick = () => {
      ...
      if (inp.trim()) {
        localTestCases.push({ input: inp, expected: exp, isCustom: true, status: 'pending' });
        saveCustomTestCases();
        renderTestCases();
      }
    };
    ...
    localTestCases.forEach((tc, idx) => {
      ...
      let metaHTML = `<span class="behelith-badge ${statusClass}">${statusText}</span>`;
      ...
      if (tc.actual !== undefined && tc.actual !== null) {
        ...
        actualDiffHTML = `
          <div class="behelith-data-block">
            <div class="behelith-data-title">Actual Output</div>
            <pre class="behelith-data-content ${diffClass}">${escapeHTML(tc.actual)}</pre>
          </div>
        `;
      }
      ...
      if (tc.stderr) {
        stderrHTML = `
          <div class="behelith-data-block">
            <div class="behelith-data-title">Standard Error</div>
            <pre class="behelith-data-content behelith-diff-error">${escapeHTML(tc.stderr)}</pre>
          </div>
        `;
      }
      ...
      card.innerHTML = `
        ...
        <div class="behelith-test-body">
          <div class="behelith-data-block">
            <div class="behelith-data-title">Input</div>
            <pre class="behelith-data-content">${escapeHTML(tc.input)}</pre>
          </div>
          <div class="behelith-data-block">
            <div class="behelith-data-title">Expected Output</div>
            <pre class="behelith-data-content">${escapeHTML(tc.expected)}</pre>
          </div>
          ${actualDiffHTML}
          ${stderrHTML}
        </div>
  ...
  function getEditorCode() {
    const nativeTextarea = document.querySelector('textarea.submission-textarea, textarea[name="body"], #editor textarea');
  ...
  function triggerLocalRun() {
    ...
    try {
      ...
      console.warn('[Behelith] Failed to send run message:', e);
    ...
    if (!panel) {
      ...
      panel.innerHTML = `
        ...
        <div class="behelith-sidebar-header">
          ...
          <div>
            <button id="behelith-sidebar-submit" class="behelith-sidebar-btn" style="margin-right: 8px;">Submit Code</button>
  ...
  window.addEventListener('message', (event) => {
    ...
    if (message.type === 'BEHELITH_SANDBOX_READY') {
      ...
      if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
        try {
          chrome.storage.local.get(['behelith_pending_submit', `behelith_code_${window.location.pathname}`], (res) => {
            if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.lastError) {
            ...
            if (res.behelith_pending_submit) {
               ...
               try {
                 chrome.storage.local.remove(['behelith_pending_submit']);
               } catch (err) {}
            ...
            if (!initialCode || initialCode.trim() === '') {
              initialCode = currentLang === 'python' ? activeSettings.pythonTemplate : activeSettings.cppTemplate;
              if (nativeTextarea) {
                   nativeTextarea.value = initialCode;
                   nativeTextarea.dispatchEvent(new Event('input', { bubbles: true }));
              }
      ...
      const template = currentLang === 'python' ? activeSettings.pythonTemplate : activeSettings.cppTemplate;
      ...
      const nativeTextarea = document.querySelector('textarea.submission-textarea, textarea[name="body"], #editor textarea');
      const ideContainer = document.querySelector('.ide-editor, #ide-editor, .cooking-editor');
      const hasNativeTextarea = !!nativeTextarea || !!ideContainer;
      if (!hasNativeTextarea) {
        toggleSidebarEditor();
      }
  ...
  function setHostLanguage(lang) {
    currentLang = lang;
    const langSelector = document.querySelector('.language-select, select.language-select, select[name="language"]');
    if (langSelector) {
      let foundIndex = -1;
      ...
      for (let i = 0; i < langSelector.options.length; i++) {
        ...
        if (lang === 'cpp') {
          if (text.includes('c++17') || text.includes('c++20') || text.includes('g++')) {
            foundIndex = i;
            break;
          }
        } else if (lang === 'python') {
          if (text.includes('py3') || text.includes('pypy 3') || text.includes('python 3')) {
            foundIndex = i;
            break;
          }
        }
      ...
      if (foundIndex === -1) {
        for (let i = 0; i < langSelector.options.length; i++) {
          const text = langSelector.options[i].text.toLowerCase();
          if (lang === 'cpp' && (text.includes('c++') || text.includes('cpp'))) {
            foundIndex = i;
            break;
  ...
  function injectMonaco() {
    ...
    if (!nativeTextarea && !ideContainer && !window.location.href.includes('problems')) return false;
    ...
    let targetElement = nativeTextarea;
    if (ideContainer) targetElement = ideContainer;
    ...
    if (targetElement) {
      ...
      const syncToMonaco = (newCode) => {
        if (iframe && iframe.contentWindow && newCode !== undefined) {
          if (nativeTextarea && nativeTextarea.dataset.behelithUpdating === 'true') return;
    ...
    if (langSelector) {
      langSelector.addEventListener('change', () => {
        const text = langSelector.options[langSelector.selectedIndex].text.toLowerCase();
        currentLang = (text.includes('python') || text.includes('py3')) ? 'python' : 'cpp';
        const iframe = document.getElementById('behelith-editor-iframe');
